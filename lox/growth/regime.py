...
def classify_growth(
    ...
    *,
    ...
    return RegimeResult(
        ...
        tags=tags,
        metrics={
            "Payrolls 3m": f"{payrolls_3m_ann:+,.0f}K" if payrolls_3m_ann is not None else None,
            "ISM Mfg": f"{ism:.1f}" if ism is not None else None,
            "Claims 4wk": f"{claims_4wk:,.0f}" if claims_4wk is not None else None,
            "IndProd YoY": f"{indpro_yoy:+.1f}%" if indpro_yoy is not None else None,
            "UNRATE": f"{unemployment_rate:.1f}%" if unemployment_rate is not None else None,
            "LEI YoY": f"{lei_yoy:+.1f}%" if lei_yoy is not None else None,
            "Payrolls MoM": f"{payrolls_mom:+.2f}%" if payrolls_mom is not None else None,
            "ISM New Ord": f"{ism_new_orders:.1f}" if ism_new_orders is not None else None,
        },
    )
