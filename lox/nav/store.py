...
from lox.utils.dates import parse_timestamp, utc_now_iso
...
def get_data_dir() -> str:
    """
    Resolve the project data directory so dashboard and CLI use the same paths.
    Prefer AOT_DATA_DIR; else project root / data (relative to this package).
    """
...
def default_nav_sheet_path() -> str:
    ...
    return os.environ.get("AOT_NAV_SHEET", "") or os.path.join(
        get_data_dir(), "nav_sheet.csv"
    )


def default_nav_flows_path() -> str:
    # CSV of user contributions/withdrawals (signed amounts).
    return os.environ.get("AOT_NAV_FLOWS", "") or os.path.join(
        get_data_dir(), "nav_flows.csv"
    )
