...
```
data/
├── nav_sheet.csv          # NAV snapshots (local only, gitignored)
├── nav_flows.csv          # Fund cash flows (local only, gitignored)
├── nav_investor_flows.csv # Investor-level flows (local only, gitignored)
...
All data files are **gitignored** and local-only. They are generated by CLI commands
