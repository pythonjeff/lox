...
from ai_options_trader.fiscal.regime import classify_fiscal_regime, classify_fiscal_regime_skeleton
from ai_options_trader.fiscal.signals import build_fiscal_deficit_page_data, build_fiscal_state
...
def register(fiscal_app: typer.Typer) -> None:
    ...
    def _deficit_level_ctx(deficit_pct_gdp: float | None) -> str:
        # Coarse: tune as you build intuition.
        return _band(
            deficit_pct_gdp,
            low=3.0,
            high=6.0,
    ...
    def _deficit_impulse_ctx(impulse_pct_gdp: float | None) -> str:
        # Negative = improving (less fiscal thrust). Positive = deteriorating.
        return _band(
            impulse_pct_gdp,
            low=-0.75,
            high=0.75,
    ...
    def _duration_share_ctx(long_share: float | None) -> str:
        # Long share is 0..1 (best-effort); higher means more duration absorption risk.
    ...
    def _tga_z_ctx(z: float | None) -> str:
        # z-score of 4w change in TGA: negative=easing (TGA down), positive=tightening (TGA up)
    ...
    def _tga_level_ctx(z_level: float | None) -> str:
        # “Is it high?” relative to recent history (rolling window used in signals).
    ...
    def _auction_tail_ctx(tail_bps: float | None) -> str:
        # Very rough, meant to be “is this getting ugly?”
    ...
    def fiscal_snapshot(
        ...
        if not full:
            ...
            # MTSDS133FMS is typically in $ millions; format into $B/$T for readability.
            ...
            # Bucket view is the primary display; 2Y/10Y sector is optional extra detail.
            ...
            # Best-effort total net issuance (can be negative in paydown months)
            net_total_m = None
            ...
            # TGA behavior (WTREGEN is typically USD millions)
            ...
            # Interpretation (lean + explainable): use z-score magnitude and sign of Δ4w.
            tga_interp = "n/a"
            ...
            print(Panel.fit(body, title="US Fiscal (MVP)", border_style="cyan"))
            return
    ...
    def fiscal_outlook(
        ...
        from ai_options_trader.llm.fiscal_outlook import llm_fiscal_outlook
