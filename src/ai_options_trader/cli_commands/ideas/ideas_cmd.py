"""
Legacy ideas commands - maintained for backward compatibility.

DEPRECATED: New code should use the clean commands in ideas_clean.py:
  - `lox ideas catalyst` instead of `lox ideas event`
  - `lox ideas screen` for ML-ranked ticker screening

These legacy commands (`event`, `macro-playbook`) are kept for users with
existing scripts or workflows that depend on them.
"""
...
from ai_options_trader.strategies.selector import choose_best_option
...
def register(ideas_app: typer.Typer) -> None:
    """Register legacy ideas commands (event, macro-playbook)."""
    ...
    def ideas_event(
        ...
        from ai_options_trader.llm.core.url_tools import fetch_url_text
