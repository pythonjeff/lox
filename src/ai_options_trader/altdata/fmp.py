...
# ============================================================================
# Financial Statement & Metrics Endpoints (Centralized)
# ============================================================================
...
def fetch_balance_sheet(
    *,
    ...
    Endpoint: /api/v3/balance-sheet-statement/{ticker}
    """
    if not settings.fmp_api_key:
        return []
    t = (ticker or "").strip().upper()
    ...
    key = f"fmp_balance_sheet_{t}_{periods}"
    p = cache_path(key)
    cached = read_cache(p, max_age=cache_max_age)
    if isinstance(cached, list):
        return cached
    ...
    import requests
    url = f"{FMP_BASE_URL}/v3/balance-sheet-statement/{t}"
    try:
        resp = requests.get(url, params={"apikey": settings.fmp_api_key, "limit": periods}, timeout=30)
        resp.raise_for_status()
        data = resp.json()
        if isinstance(data, list):
            write_cache(p, data[:periods])
            return data[:periods]
    except Exception:
        pass
    return []
...
def fetch_income_statement(
    *,
    ...
    key = f"fmp_income_statement_{t}_{periods}"
    p = cache_path(key)
    cached = read_cache(p, max_age=cache_max_age)
    if isinstance(cached, list):
        return cached
    ...
    import requests
    url = f"{FMP_BASE_URL}/v3/income-statement/{t}"
    try:
        resp = requests.get(url, params={"apikey": settings.fmp_api_key, "limit": periods}, timeout=30)
        resp.raise_for_status()
        data = resp.json()
...
def fetch_cash_flow(
    *,
    ...
    Endpoint: /api/v3/cash-flow-statement/{ticker}
    """
    if not settings.fmp_api_key:
        return []
    t = (ticker or "").strip().upper()
    ...
    key = f"fmp_cash_flow_{t}_{periods}"
    p = cache_path(key)
    cached = read_cache(p, max_age=cache_max_age)
    if isinstance(cached, list):
        return cached
    ...
    import requests
    url = f"{FMP_BASE_URL}/v3/cash-flow-statement/{t}"
    try:
        resp = requests.get(url, params={"apikey": settings.fmp_api_key, "limit": periods}, timeout=30)
        resp.raise_for_status()
        data = resp.json()
...
def fetch_key_metrics(
    *,
    ...
    Endpoint: /api/v3/key-metrics-ttm/{ticker} or /api/v3/key-metrics/{ticker}
    """
    if not settings.fmp_api_key:
        return {}
    t = (ticker or "").strip().upper()
    ...
    endpoint = "key-metrics-ttm" if ttm else "key-metrics"
    key = f"fmp_{endpoint}_{t}"
    p = cache_path(key)
    cached = read_cache(p, max_age=cache_max_age)
    if isinstance(cached, dict):
        return cached
    if isinstance(cached, list) and cached:
        return cached[0] if isinstance(cached[0], dict) else {}
    
    import requests
    url = f"{FMP_BASE_URL}/v3/{endpoint}/{t}"
    try:
        resp = requests.get(url, params={"apikey": settings.fmp_api_key}, timeout=30)
        resp.raise_for_status()
        data = resp.json()
...
def fetch_ratios(
    *,
    ...
    Endpoint: /api/v3/ratios-ttm/{ticker} or /api/v3/ratios/{ticker}
    """
    if not settings.fmp_api_key:
        return {}
    t = (ticker or "").strip().upper()
    ...
    endpoint = "ratios-ttm" if ttm else "ratios"
    key = f"fmp_{endpoint}_{t}"
    p = cache_path(key)
    cached = read_cache(p, max_age=cache_max_age)
    if isinstance(cached, dict):
...
def fetch_financial_growth(
    *,
    ...
    Endpoint: /api/v3/financial-growth/{ticker}
    """
    if not settings.fmp_api_key:
        return []
    t = (ticker or "").strip().upper()
    ...
    key = f"fmp_financial_growth_{t}_{periods}"
    p = cache_path(key)
    cached = read_cache(p, max_age=cache_max_age)
    if isinstance(cached, list):
        return cached
    ...
    import requests
    url = f"{FMP_BASE_URL}/v3/financial-growth/{t}"
    try:
        resp = requests.get(url, params={"apikey": settings.fmp_api_key, "limit": periods}, timeout=30)
        resp.raise_for_status()
        data = resp.json()
...
def fetch_analyst_estimates(
    *,
    ...
    key = f"fmp_analyst_estimates_{t}_{periods}"
    p = cache_path(key)
    cached = read_cache(p, max_age=cache_max_age)
    if isinstance(cached, list):
        return cached
    ...
    import requests
    url = f"{FMP_BASE_URL}/v3/analyst-estimates/{t}"
    try:
        resp = requests.get(url, params={"apikey": settings.fmp_api_key, "limit": periods}, timeout=30)
        resp.raise_for_status()
        data = resp.json()
...
def fetch_etf_holdings(
    *,
    ...
    Endpoint: /api/v3/etf-holder/{ticker}
    """
    if not settings.fmp_api_key:
        return []
    t = (ticker or "").strip().upper()
    ...
    import requests
    url = f"{FMP_BASE_URL}/v3/etf-holder/{t}"
    try:
        resp = requests.get(url, params={"apikey": settings.fmp_api_key}, timeout=30)
        resp.raise_for_status()
        data = resp.json()
...
def fetch_institutional_holders(
    *,
    ...
    Endpoint: /api/v3/institutional-holder/{ticker}
    """
    if not settings.fmp_api_key:
        return []
    t = (ticker or "").strip().upper()
    ...
    key = f"fmp_institutional_holders_{t}"
    p = cache_path(key)
    cached = read_cache(p, max_age=cache_max_age)
    if isinstance(cached, list):
        return cached
    ...
    import requests
    url = f"{FMP_BASE_URL}/v3/institutional-holder/{t}"
    try:
        resp = requests.get(url, params={"apikey": settings.fmp_api_key}, timeout=30)
        resp.raise_for_status()
        data = resp.json()
...
def fetch_price_target(
    *,
    ...
    import requests
    url = f"{FMP_BASE_URL}/v4/price-target-consensus"
    try:
        resp = requests.get(url, params={"symbol": t, "apikey": settings.fmp_api_key}, timeout=30)
        resp.raise_for_status()
        data = resp.json()
        if isinstance(data, list) and data:
            result = data[0] if isinstance(data[0], dict) else {}
