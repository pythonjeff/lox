...
# ============ TRADE THESIS INDICATORS CONFIG ============
# Key format: "{underlying}_{strike}_{opt_type}_{expiry}" for options
# Or raw symbol for equity/crypto positions. Add indicators per trade.
TRADE_INDICATORS_CONFIG = {
    # ═══════════════════════════════════════════════════════
    # EQUITY / ETF POSITIONS
    # ═══════════════════════════════════════════════════════

    # AMZN (Long 3 shares) — Growth / AI / Cloud / Consumer
    "AMZN": [
    ...
    # BE (Long 4 shares) — Bloom Energy / Data Center Power / Clean Energy
    "BE": [
    ...
    # SPXU (Long 10 shares) — 3× Inverse S&P 500
    "SPXU": [
    ...
    # VIXY (Long 41 shares) — VIX Short-Term Futures ETF
    "VIXY": [
        {"name": "VIX", "current_value": "15.5", "target_value": ">25", "invalidation_value": "<12", "direction": "above"},
    ...
    # SPY $550P Jun 18 2026 (2 contracts) — Broad equity correction
    ...
    # HYG $74P Jul 17 2026 (8 contracts) — High-yield credit crunch
    "HYG_74_P_2026-07-17": [
    ...
    # VIXY $30C Mar 20 2026 (1 contract) — Leveraged vol spike
    ...
    # TAN $32P Jun 18 2026 (2 contracts) — Solar sector weakness
    "TAN_32_P_2026-06-18": [
    ...
    # CRWV $45P Aug 21 2026 (1 contract) — CoreWeave overvaluation
    ...
    # SNDK $150P Mar 20 2026 (1 contract, ~$5 MV) — Memory cycle downturn
    ...
    # ═══════════════════════════════════════════════════════
    # TEMPLATE (not a live position — user example from original request)
    # ═══════════════════════════════════════════════════════
    "TLT_86_P_2026-10-17": [
...
def _position_indicator_key(position_dict):
    """Build lookup key for TRADE_INDICATORS_CONFIG from position. Returns None if not an option."""
...
def _indicator_status(indicator):
    """
    Compute status based on how well the indicator supports our trade thesis.

    Maps current value onto a 0-1 scale between invalidation (0) and target (1):
        >= 0.7  → green  (thesis is working)
        0.3-0.7 → yellow (neutral / inconclusive)
        < 0.3   → red    (thesis is under pressure)

    Direction "above": target > current > invalidation  (we want it to rise)
    Direction "below": target < current < invalidation  (we want it to fall)
    """
    direction = (indicator.get("direction") or "above").lower()
    curr_val = indicator.get("current_value") or indicator.get("current")
    tgt_val = indicator.get("target_value") or indicator.get("target")
    inv_val = indicator.get("invalidation_value") or indicator.get("invalidation")
    curr = _parse_numeric(curr_val)
    tgt = _parse_numeric(tgt_val)
    inv = _parse_numeric(inv_val)
    if curr is None:
        return "neutral"
    ...
    if direction == "above":
        # We want curr to be high (toward tgt, away from inv)
        if curr >= tgt:
            return "green"
        if curr <= inv:
            return "red"
        ...
        if span <= 0:
            return "neutral"
        pct = (curr - inv) / span  # 0 = at invalidation, 1 = at target
    else:
        # We want curr to be low (toward tgt, away from inv)
        if curr <= tgt:
            return "green"
        if curr >= inv:
            return "red"
        ...
        if span <= 0:
            return "neutral"
        pct = (inv - curr) / span  # 0 = at invalidation, 1 = at target
    ...
    try:
        ...
        for pd in position_data:
            try:
                ...
                # Thesis indicators from TRADE_INDICATORS_CONFIG (per-trade config)
                position_dict["indicators"] = _get_indicators_for_position(position_dict)
